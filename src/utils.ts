...
export function npm(
  ...
  return new Promise((resolve, reject) => {
    const useYarn = hasYarn(projectPath);
    command = useYarn ? "" : command;
    const procName = useYarn ? "yarn" : "npm";
    const p = spawn(procName, [command], {
      shell: true,
      stdio,
      cwd: projectPath
    });
      ...
      console.log(
        tc.yellow(
          `Your current version of Node is ${
            process.version
          }, however the recommendation is a minimum of Node 8.x LTS. Note that future versions of Stencil will eventually remove support for non-LTS Node versions.`
        )
      );
    }
  } catch (e) {
